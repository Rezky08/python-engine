...
from mata_kuliah.step3.rules_matkul_new import rules
import random
import numpy as np
import time
import threading
...
class algen_matkul():
    ...
    def generate_first_pop(self):
        ...
        for num_population in range(self.num_population):
            chromosom = random.choices(self.simply_nn_params['sesi'],k=len(self.nn_params['mata_kuliah']))
        ...
        for index,gen in enumerate(chromosom):
            if self.mutation_rate>random.random():
                ...
                while True:
                    ...
                    choosed = random.choice(self.simply_nn_params['sesi'])
                    ...
                    if len(choosed_list) >= len(self.simply_nn_params['sesi']):
                        break
    ...
    def translate(self, chromosom):
        for index,gen in enumerate(chromosom):
            mata_kuliah = copy.deepcopy(self.nn_params['mata_kuliah'][index])
            sesi_mulai = gen
            sesi_selesai = mata_kuliah['sks_matkul']+gen-1
            chromosom[index]= {
                'hari' : self.nn_params['hari'][mata_kuliah['hari']],
                'ruang' : self.nn_params['ruang'][mata_kuliah['ruang']]['nama_ruang'],
            }
            if sesi_selesai > len(self.nn_params['sesi'])-1:
                ...
                sesi = {
                    'sesi_mulai' : self.nn_params['sesi'][sesi_mulai]['sesi_mulai'].strftime("%H:%M:%S"),
                    'sesi_selesai' : self.nn_params['sesi'][sesi_selesai]['sesi_selesai'].strftime("%H:%M:%S")
                }
                chromosom[index] = {
                    **chromosom[index],**sesi
                }
